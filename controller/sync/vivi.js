...
class ViviSync {
  ...
  start(isInit) {
    if(isInit) {
      wordModel.update({viviSyncStatus: 'syncing'}, {viviSyncStatus: 'sync'}, (err) => {
  ...
  getSyncList(num, cb) {
    wordModel.find({where: {viviSyncStatus: 'sync'}, limit: num || 100}, (err, data) => {
      if(err) return cb(err)
      if(data.length === 0) return cb(null, data)

      let idList = _.map(data, 'id')
      wordModel.update(SqlBricks.in('id', idList), { viviSyncStatus: 'syncing' }, (err) => {
        cb(err, data)
      })
    })
  }
